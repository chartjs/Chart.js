    ...
    [undefined, 20].forEach(function(barThickness) {
      describe('When barThickness is ' + barThickness, function() {
        ...
        it('should correctly set bar width if minBarThickness is specified', function() {
          ...
          chart.data.datasets[0].minBarThickness = 100;
          chart.data.datasets[1].minBarThickness = 100;
          chart.update();

          for (i = 0, ilen = chart.data.datasets.length; i < ilen; ++i) {
            meta = chart.getDatasetMeta(i);
            expect(meta.data[0].width).toBeCloseToPixel(100);
            expect(meta.data[1].width).toBeCloseToPixel(100);
          }
        });
      });
    });
