...
export function isArray(value) {
	...
	const type = Reflect.apply(Object.prototype.toString, value, []);
...
export function isObject(value) {
	return value !== null && Reflect.apply(Object.prototype.toString, value, []) === '[object Object]';
}
...
export function each(loopable, fn, thisArg, reverse) {
	...
	if(isArray(loopable)) {
		...
		if(reverse) {
			for(i = len - 1; i >= 0; i--) {
				Reflect.apply(fn, thisArg, [loopable[i], i]);
			}
		} else {
			for(i = 0; i < len; i++) {
				Reflect.apply(fn, thisArg, [loopable[i], i]);
		...
		len = keys.length;
		for(i = 0; i < len; i++) {
			Reflect.apply(fn, thisArg, [loopable[keys[i]], keys[i]]);
		}
	}
}
