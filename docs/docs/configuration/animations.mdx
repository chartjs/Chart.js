...
Mode option configures how an update mode must be animated. The cores Core modes are `'active'`, `'hide'`, `'reset'`, `'resize'`, `'show'`. A custom mode can be used by passing a custom `mode` to [update](../developers/api.md#updatemode).
...
Property option configures which element property to use to animate the chart.
...
| `to`  | <code>`number`&#124;`Color`&#124;`boolean`</code> | `undefined` | End value for the animation. Current value is used when `undefined`

## Animation properties collection configuration

Properties collection option configures which a set of properties to use to animate the chart.  Collection can be named whatever you like, but should not collide with a `[property]` or `[mode]`.
...
To enable a custom collection, the default one of the same type must be disabled.
...
chart.options.animation.active = false; // disables the animation for 'active' mode
chart.options.animation.colors = false; // disables the collection of colors properties
```
