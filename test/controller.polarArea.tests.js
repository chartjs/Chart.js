...
describe('Polar area controller tests', function() {
	...
	it('should update elements when modifying data', function() {
		...
		for (var i = 0; i < 4; ++i) {
			expect(meta.data[i]._model.backgroundColor).toBe('rgba(128, 129, 130, 1)');
			expect(meta.data[i]._model.borderColor).toBe('rgba(56, 57, 58, 1)');
		...
		expect(meta.data[0]._model).toEqual(jasmine.objectContaining({
			...
			endAngle: 0,
			backgroundColor: 'rgba(0, 1, 3, 1)',
			borderWidth: 0.787,
			borderColor: 'rgba(4, 6, 8, 1)',
			label: 'label1'
		}));
	});
	...
	it('should set arc hover styles', function() {
		...
		expect(arc._model.borderColor).toBe('rgba(0, 230, 0, 1)');
		expect(arc._model.borderWidth).toBe(1.2);

		// Can set hover style per dataset
		chart.data.datasets[0].hoverBackgroundColor = 'rgba(77, 79, 81, 1)';
		chart.data.datasets[0].hoverBorderColor = 'rgba(123, 125, 127, 1)';
		chart.data.datasets[0].hoverBorderWidth = 2.1;

		meta.controller.setHoverStyle(arc);
		expect(arc._model.backgroundColor).toBe('rgba(77, 79, 81, 1)');
		expect(arc._model.borderColor).toBe('rgba(123, 125, 127, 1)');
		...
		meta.controller.setHoverStyle(arc);
		expect(arc._model.backgroundColor).toBe('rgba(0, 0, 0, 1)');
		expect(arc._model.borderColor).toBe('rgba(10, 10, 10, 1)');
	...
	it('should remove hover styles', function() {
		...
		expect(arc._model.backgroundColor).toBe('rgba(45, 46, 47, 1)');
		expect(arc._model.borderColor).toBe('rgba(50, 51, 52, 1)');
		expect(arc._model.borderWidth).toBe(10.1);

		// Can set hover style per dataset
		chart.data.datasets[0].backgroundColor = 'rgba(77, 79, 81, 1)';
		chart.data.datasets[0].borderColor = 'rgba(123, 125, 127, 1)';
