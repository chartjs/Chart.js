...
The animation configuration is passed into the `options.animation` namespace. The global options are defined in `Chart.defaults.animation`.
...
| ---- | ---- | ------- | -----------
| `animation.duration` | `number` | `1000` | The number of milliseconds an animation takes.
| `animation.easing` | `string` | `'easeOutQuart'` | Easing function to use. [more...](#easing)
| `animation.debug` | `boolean` | `undefined` | Running animation count + FPS display in upper left corner of the chart.
| `animation.delay` | `number` | `undefined` | Delay before starting the animations.
| `animation.loop` | `boolean` | `undefined` | If set to `true`, the animations loop endlessly.
| [animation[mode]](#animation-mode-configuration) | `object` | [defaults...](#default-modes) | Option overrides for update mode. Core modes: `'active'`, `'hide'`, `'reset'`, `'resize'`, `'show'`. See **Hide and show [mode]** example above.
| [animation[property]](#animation-property-configuration) | `object` | `undefined` | Option overrides for a single element `[property]`. These have precedence over `[collection]`. See **Looping tension [property]** example above.
| [animation[collection]](#animation-properties-collection-configuration) | `object` | [defaults...](#default-collections) | Option overrides for multiple properties, identified by `properties` array.
...
| -----| ------ | ----- | -----
| `animation.active` | duration | 400 | Override default duration to 400ms for hover animations
| `animation.resize` | duration | 0 | Override default duration to 0ms (= no animation) for resize
| `animation.show` | colors | `{ type: 'color', properties: ['borderColor', 'backgroundColor'], from: 'transparent' }` | Colors are faded in from transparent when dataset is shown using legend / [api](../developers/api.md#showdatasetIndex).
| `animation.show` | visible | `{ type: 'boolean', duration: 0 }` | Dataset visiblity is immediately changed to true so the color transition from transparent is visible.
| `animation.hide` | colors | `{ type: 'color', properties: ['borderColor', 'backgroundColor'], to: 'transparent' }` | Colors are faded to transparent when dataset id hidden using legend / [api](../developers/api.md#hidedatasetIndex).
| `animation.hide` | visible | `{ type: 'boolean', easing: 'easeInExpo' }` | Visibility is changed to false at a very late phase of animation
...
| Name | Type | Default | Description
| ---- | ---- | ------- | -----------
| `animation[animation].type` | `string` | `typeof property` | Type of property, determines the interpolator used. Possible values: `'number'`, `'color'` and `'boolean'`. Only really needed for `'color'`, because `typeof` does not get that right.
| `animation[animation].from`  | `number`\|`Color`\|`boolean` | `undefined` | Start value for the animation. Current value is used when `undefined`
| `animation[animation].to`  | `number`\|`Color`\|`boolean` | `undefined` | End value for the animation. Updated value is used when `undefined`
| `animation[animation].fn` | <code>&lt;T&gt;(from: T, to: T, factor: number) => T;</code> | `undefined` | Optional custom interpolator, instead of using a predefined interpolator from `type` |
...
| Name | Type | Default | Description
| ---- | ---- | ------- | -----------
| `animation[collection].properties` | `string[]` | `undefined` | Set of properties to use to animate the chart.
...
| Name | Type | Default | Description
| ---- | ---- | ------- | -----------
| `animation.onProgress` | `function` | `null` | Callback called on each step of an animation.
| `animation.onComplete` | `function` | `null` | Callback called when all animations are completed.
