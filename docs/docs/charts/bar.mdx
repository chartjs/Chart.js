...
| [`borderRadius`](#borderradius) | The bar border radius. If this value is a number for pixels(`5` = `'5px'`) or is a string for percentage(`'50'` = `'50%'` base of the minimum width and height of the bar). All of the values are going to be converted to pixels.
| `clip` | How to clip relative to chartArea. Positive value allows overflow, negative value clips that many pixels inside chartArea. `0` = clip at chartArea. Clipping can also be configured per side: `clip: {left: 5, top: false, right: -2, bottom: 0}`
